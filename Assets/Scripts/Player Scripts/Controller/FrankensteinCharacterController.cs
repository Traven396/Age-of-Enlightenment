...
public class FrankensteinCharacterController : MonoBehaviour
{
    ...
    #region Settings
    [Header("Any other settings")]
    //How fast the player rotates
    public float turnSpeed = 10f;
    //The threshold for when the player rotates
    [Range(0, 1)]
    ...
    //the maximum amount gravity can reach
	...
	void Awake()
	{
        ...
        movementSpeed = Player.Instance.movementSpeed.Value;
        Player.StatsChanged.AddListener(SetSpeed);
    }
    ...
    void ControllerUpdate()
    {
        ...
        //Rotate the character if they are pushing the joystick
        HandleTurning();
    ...
    protected virtual Vector3 CalculateMovementDirection()
    {
        ...
        var inputVector = MoveJoystick.action.ReadValue<Vector2>();
    ...
    {
        return JumpButton.action.WasPressedThisFrame();
    }
    ...
    void HandleTurning()
    {
        ...
        {
            mover.RotatePlayer(inputValue * turnSpeed);
        }
    }
    #endregion
    #region Events
    ...
    public void ChangeGravity(float newValue, bool allowNegative)
    {
        if (allowNegative)
        {
            gravity += newValue; 
        ...
        {
            gravity = Mathf.Max(gravity + newValue, 0);
        }
