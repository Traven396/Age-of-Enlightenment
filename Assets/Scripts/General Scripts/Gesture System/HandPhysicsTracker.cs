namespace FoxheadDev.GestureDetection
{
	...
	public class HandPhysicsTracker
	{
		...
		public Transform PlayerBodyTransform { get; private set; }
        public HandPhysicsTracker OtherHandTracker { get; private set; }
		public bool LeftHand { get; private set; } = false;

        #region Constructors + Setters
        public HandPhysicsTracker(Transform handTransform, Transform playerBody, bool flipXForLeftHand)
        {
        ...
        public void SetOtherHand(HandPhysicsTracker newOtherHand)
        {
            OtherHandTracker = newOtherHand;
        }
        #endregion
        #region Constants
        /// <summary>
		...
		#region Output
		// Output data
        //This is the direction out of the palm. This way it will work either hand its used on.
        public Vector3 UniveralPalm { get
            {
                if (LeftHand)
                    return SelfHandTransform.right;
                else
                    return -SelfHandTransform.right;
            } }
        public float DistanceToOtherHand { get
            {
                return Vector3.Distance(SelfHandTransform.position, OtherHandTracker.SelfHandTransform.position);
            } }
        ...
        public Vector3 SelfSpaceVelocity => LeftHand ? SelfHandTransform.InverseTransformVector(Velocity.InvertX()) : SelfHandTransform.InverseTransformVector(Velocity);
        ...
        public Vector3 SelfSpaceAngularVelocity { get => LeftHand ? SelfHandTransform.InverseTransformDirection(AngularVelocity.InvertY()) : SelfHandTransform.InverseTransformDirection(AngularVelocity); }
		...
		public void Update(Vector3 newPosition, Quaternion newRotation, float timeSlice)
		{
            ...
            var currentOffset = (newPosition - m_LastOffsetPosition) - (PlayerBodyTransform.position - m_LastBodyOffsetPosition);
            var currentDistance = currentOffset.magnitude;

            m_LastBodyOffsetPosition = PlayerBodyTransform.position;
