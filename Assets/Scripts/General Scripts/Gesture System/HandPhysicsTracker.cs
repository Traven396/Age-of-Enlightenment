namespace AgeOfEnlightenment.GestureDetection
{
	...
	public class HandPhysicsTracker
	{
		...
		public HandPhysicsTracker(Transform handTransform, Transform playerBody)
		{
		...
		/// <summary>
		/// The distance in front of the hand to check for finger velocity at. Bigger values means that Flicking will be much easier
		/// </summary>
		const float k_FingerDistance = 0.01f;
		/// <summary>
		...
		public Vector3 FingerLinearVelocity { get
            {
				Vector3 fingerPos = Vector3.forward * k_FingerDistance;
				Vector3 tempVel = Vector3.Cross(AngularVelocity, fingerPos);

				tempVel = SelfHandTransform.TransformDirection(tempVel);
				tempVel += Velocity;
		...
		public Vector3 FingerAngularVelocity => AngularVelocity * k_FingerDistance;
